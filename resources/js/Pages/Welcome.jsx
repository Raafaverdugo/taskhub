export default function Welcome() {
    ...
    const nombreUsuario = "Rafael Verdugo";
    const tareasPendientes = ["Instalar Laravel", "Configurar React", "Aprender Tailwind", "Hacer web instituto"];
                ...
                <ul className="space-y-3">
                    {tareasPendientes.map((tarea, index) => (
                        <li key={index} className="flex items-center gap-3 p-2 bg-slate-50 rounded-lg">
                            ...
                            <span className={tarea.toLowerCase().includes('laravel') ? 'text-blue-500' : ''}>
                                {tarea}
                            </span>
                        </li>
                    ))}
